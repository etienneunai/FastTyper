...
* **`src/background.ts`**: All daemon traffic goes through here — with `host_permissions` for `127.0.0.1:8808`, the background context can fetch localhost without the page's CORS policy (a content script cannot). Also owns settings (`storage.local`: pause, capitalize-sentence-initials, **site blacklist**, **prompt selection** — `promptId` + `customSystem`/`customUser`; `correct()` resolves the active prompt via `resolvePrompt` before building the payload), the daemon-health probe, the `Ctrl+Shift+F` pause command, and a 50-entry exchange-log ring buffer (a browser extension has no filesystem access, so there is **no `llm-log.txt`** — log is viewable in the popup).
...
- **E — proof**: System "You are a careful proofreader." · User "Fix only clear errors: misspellings, run-together words, missing apostrophes, and a/an agreement. Never reword, restyle, or alter correct text…". Most capable, but emits a ~340-token reasoning block every request (6–25 s per correction) — a real-time risk.
